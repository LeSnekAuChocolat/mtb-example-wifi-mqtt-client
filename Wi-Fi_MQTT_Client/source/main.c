...
int main()
{
    ...
    result = cyhal_gpio_init(TESTBUTTON, CYHAL_GPIO_DIR_INPUT, 
                            CYHAL_GPIO_DRIVE_PULLUP, true);
    
    // Initialize LED as bidirectional since we need to read the value to toggle. Initialized with off state by default
    result = cyhal_gpio_init(TESTLED, CYHAL_GPIO_DIR_BIDIRECTIONAL, CYHAL_GPIO_DRIVE_PULLUP, 
    						CYBSP_LED_STATE_OFF);
