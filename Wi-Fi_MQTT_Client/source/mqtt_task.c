...
static cy_rslt_t mqtt_get_unique_client_identifier(char *mqtt_client_identifier);
 ...
 //Bitmask button debouncing method
static bool is_button_pressed(void) {
    static uint8_t button_history = 0xFF;  // Start with all 0s (button not pressed), change the type to uint16_t or more for more accuracy
    static bool last_stable_state = true;  // true = not pressed, default state
    
    const uint8_t DEBOUNCE_MASK = 0x07;  // Increase if accuracy is not sufficient
    
    // Shift history left and add current reading
    button_history = (button_history << 1) | (cyhal_gpio_read(TESTBUTTON) ? 1 : 0);
    
    // Check for stable LOW (button pressed)
    if ((button_history & DEBOUNCE_MASK) == 0x00) {
        // All recent readings are LOW (button is stable pressed)
        if (last_stable_state == true) {
            // Falling edge detected: was HIGH, now stable LOW
            last_stable_state = false;
            printf("Button Pressed!\n");
            printf("%x\n",button_history);
            return true;  // Button press event!
        }
    }
    // Check for stable HIGH (button released)
    else if ((button_history & DEBOUNCE_MASK) == DEBOUNCE_MASK) {
        // All recent readings are HIGH (button is stable released)
        last_stable_state = true;  // Ready for next press
    }
    return false;  // No button press event
}

static void toggle_led(void) {
	bool state = cyhal_gpio_read(TESTLED);
	cyhal_gpio_write(TESTLED, !state);
}
...
void mqtt_client_task(void *pvParameters)
{
    ...
    while (true)
    {	//checks if TESTBUTTON is pressed and toggles the TESTLED, using debouncing algorithm
		if(is_button_pressed()){
			toggle_led();
			}
        /* Wait for results of MQTT operations from other tasks and callbacks. */
        //changed from portMAX_Delay to a smaller value, allowing to poll in while()
        if (pdTRUE == xQueueReceive(mqtt_task_q, &mqtt_status, 0x8f))
        {
